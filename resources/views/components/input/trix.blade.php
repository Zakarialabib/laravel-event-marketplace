<div class="mb-5">
    ...
    <input id="{{ $id }}" type="hidden" name="{{ $name }}" x-ref="{{ $name }}"
    ...
    class="relative" x-cloak>
        ...
        <trix-editor placeholder="{{ $placeholder ?? '' }}" input="{{ $id }}"
            class="trix-editor border-gray-300 trix-content"
            :class="{ ' border-red-500 bg-red-100': error.length || '{{ $errors->has($name) }}' }" x-ref="trix-editor"
            x-on:trix-change="$dispatch('input', event.target.value)" x-on:keydown="error.length ? error = '' : ''"
           ...
           "
            ...
            x-on:trix-attachment-remove="deleteAttachment(event)"></trix-editor>
