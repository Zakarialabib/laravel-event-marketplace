<div>
            ...
            <div class="mb-5 lg:mb-0">
                ...
                <div class="flex items-center">
                    ...
                    <a class="flex items-center text-sm" href="{{ URL::Current() }}">
                        ...
                        <span>{{ __('Race Report') }}</span>
                    </a>
                </div>
            </div>
            {{-- <div class="float-right">
                <button wire:click="exportExcel"
                    class="mx-1 px-4 py-2 bg-green-500 text-white rounded-md hover:bg-green-600">{{ __('Export as Excel') }}</button>
                <button onclick="window.print()"
                    class="mx-1 px-4 py-2 bg-indigo-500 text-white rounded-md hover:bg-indigo-600">{{ __('Print') }}</button>
            </div> --}}
        </div>
    </section>
    ...
    <div class="container mx-auto p-6">
        <div class="mb-4">
            <!-- Advanced Search -->
            <input type="text" placeholder="{{ __('Search by name or location') }}"
                wire:model.debounce.300ms="filters.search"
                class="w-full mt-4 px-4 py-2 border rounded-md focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
            <!-- Enhanced Filtering & Multi-select Filters -->
            <div class="flex gap-4 my-auto py-4 items-center">
                <div class="flex flex-wrap gap-4">
                    <x-label for="status" :value="__('Status')" />
                    @foreach (\App\Enums\RaceStatus::values() as $value => $label)
                        <p class="text-sm"><input class="mr-2" type="checkbox" value="{{ $value }}"
                                wire:model="filters.statuses">{{ $label }}
                        </p>
                    @endforeach
                </div>
                ...
                <div>
                    ...
                    <select wire:model="filters.location" class="border rounded-md">
                ...
                <div>
                    <x-label for="status" :value="__('Date from:')" />
                    <input type="date" wire:model="filters.dateFrom" class="border rounded-md">
                </div>
                <div>
                    <x-label for="status" :value="__('Date To:')" />
                    <input type="date" wire:model="filters.dateTo" class="border rounded-md">
                </div>
                <x-button wire:click="resetFilters" type="button" danger class="py-2">
                    <i class="fa fa-times"></i>
                </x-button>
            </div>
        </div>
        <!-- Data Insights -->
        <div class="flex justify-between bg-gray-50 px-4 py-2 border border-gray-700">
            <span>{{ __('Total Races:') }} {{ $totalRaces }}</span>
            <span>{{ __('Upcoming Races:') }} {{ $upcomingRaces }}</span>
            <span>{{ __('Registrations:') }} {{ $registrations }}</span>
        </div>
            ...
            <x-table.tbody>
                @foreach ($races as $race)
                    <tr>
                        ...
                        <td class="py-2 px-4 border-b border-gray-200">{{ $race->totalOrders }}</td>
                    </tr>
                @endforeach
            </x-table.tbody>
        </x-table>
