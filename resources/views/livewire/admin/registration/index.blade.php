<div>
    ...
    <x-table>
        ...
        <x-table.tbody>
            @forelse($registrations as $id=>$registration)
                <x-table.tr>
                    <x-table.td>
                        {{ $id }}
                    </x-table.td>
                    <x-table.td>
                        {{ $registration->name }} - {{ $registration->phone }} <br>
                        {{ $registration->address }}
                    </x-table.td>
                    <x-table.td>
                        {{ $registration->subject }} <br>
                        {{ $registration->message }}
                    </x-table.td>
                    <x-table.td>
                        @if($registration->status == 0)
                        <livewire:toggle-button :model="$registration" field="status" key="{{ $registration->id }}" />
                        @else
