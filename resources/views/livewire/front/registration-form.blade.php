<div>
    <form wire:submit.prevent="register" class="mx-auto mb-4 ">
        <h3
            class="w-full text-center mb-6 pt-10 pb-5    text-3xl lg:text-5xl md:text-3xl sm:text-xl font-bold uppercase text-gray-800">
        ...
        <div x-data="{ step: 1, selectedTeam: null, teamName: @entangle('team_name'), invitationEmails: [''] }" class="border rounded p-4  shadow-lg w-full text-center mb-4">
            ...
            @if ($isTeamRegistration)
                ...
                <div x-show="step === 1" class="mb-4 px-6">
                    <label class="block mb-2 font-bold text-gray-700">Team Name</label>
                    <input type="text" wire:model="team_name" placeholder="Enter or search team name"
                    ...
                    <!-- Dropdown for existing teams -->
                    <ul class="mt-2 space-y-1">
                        @foreach (\App\Models\Team::all() as $team)
                            <li x-show="!selectedTeam" class="px-3 py-1 hover:bg-gray-200 cursor-pointer"
                                @click="selectedTeam = '{{ $team->team_name }}'; step = 3;">
                                {{ $team->team_name }}
                            </li>
                        @endforeach
                    </ul>
                    <x-button secondary x-on:click="step = 2" type="button" class="py-4">
                        {{ __('Create New Team') }}
                    </x-button>
                </div>
                ...
                <div x-show="step === 2" class="mb-4 px-6">
                    <label class="block mb-2 font-bold text-gray-700">Create a New Team</label>
                    <input type="text" wire:model="newTeamName" placeholder="Enter new team name"
                        class="w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:border-blue-300 focus:ring focus:ring-blue-200">
                    <x-button primary x-on:click="step = 3" type="button" class="mt-2">
                        {{ __('Confirm New Team') }}
                    </x-button>
                </div>

                <!-- Step 3: Invite Team Members by Email -->
                <div x-show="step === 3" class="mb-4 px-6">
                    <label class="block mb-2 font-bold text-gray-700">Invite Team Members by Email</label>
        ...
        <div class="bg-white rounded p-4 mx-6">
            ...
            <div class="grid xl:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-4 items-cente">
                ...
                <div>
                    ...
                    <x-input wire:model="participant.country" type="text" required id="country" name="country" />
                ...
                <div>
                    ...
                    <x-input wire:model="participant.city" required type="text" id="city" name="city" />
            ...
            <div class="col-span-full">
                ...
                <x-checkbox-input label="{{ __('Health Informations') }}" model="{{ $race->health_informations }}">
                ...
                <x-checkbox-input label="{{ __('Taking Medications') }}" model="{{ $race->taking_medications }}">
                ...
                <x-checkbox-input label="{{ __('has Medication Allergies') }}"
                    ...
                    <x-input wire:model="participant.medication_allergies" type="text" id="medication_allergies"
                        class="w-full block" name="medication_allergies" />
                </x-checkbox-input>
                    ...
                    <li>
                        <label for="service-{{ $service->id }}" class="flex items-center">
                            <input type="checkbox" wire:model="selectedServices" name="services[]"
                                id="service-{{ $service->id }}" value="{{ $service->id }}" class="mr-2">
                            {{ $service->name }} ({{ $service->price }})
                            <!-- Display the price as well, if needed -->
                        </label>
                    </li>
                @endforeach
            </ul>

            <div class="w-full grid grid-cols-2 sm:grid sm:grid-cols-1 py-2 gap-2 justify-between sm:justify-center">
